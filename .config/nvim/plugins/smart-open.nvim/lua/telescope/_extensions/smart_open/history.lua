...
-- Git worktree path canonicalization.
-- All paths in the DB use the main repo prefix (canonical form).
-- When in a worktree, paths are translated on read/write so that
-- frecency is shared across all worktrees and the main repo.
local worktree_info_cached = nil
local worktree_info_resolved = false

local function get_worktree_info()
  if worktree_info_resolved then
    return worktree_info_cached
  end
  worktree_info_resolved = true
  ...
  local git_dir = git_cmd("rev-parse --git-dir")
  local common_dir = git_cmd("rev-parse --git-common-dir")
  local toplevel = git_cmd("rev-parse --show-toplevel")
  if not git_dir or not common_dir or not toplevel then
    return nil
  end

  local abs_git = vim.fn.resolve(git_dir)
  local abs_common = vim.fn.resolve(common_dir)
  if abs_git == abs_common then
    return nil -- not a worktree
  end

  local main_repo = vim.fn.fnamemodify(abs_common, ":h")
  if main_repo:sub(-1) ~= "/" then
    main_repo = main_repo .. "/"
  end
  if toplevel:sub(-1) ~= "/" then
    toplevel = toplevel .. "/"
  end

  worktree_info_cached = { main_repo = main_repo, worktree_root = toplevel }
  return worktree_info_cached
end

--- Replace worktree prefix with main repo prefix (for DB storage)
local function to_canonical(filepath)
  local info = get_worktree_info()
  if not info then
    return filepath
  end
  if filepath:sub(1, #info.worktree_root) == info.worktree_root then
    return info.main_repo .. filepath:sub(#info.worktree_root + 1)
  end
  return filepath
end

--- Replace main repo prefix with worktree prefix (for display/opening)
local function from_canonical(filepath)
  local info = get_worktree_info()
  if not info then
    return filepath
  end
  if filepath:sub(1, #info.main_repo) == info.main_repo then
    return info.worktree_root .. filepath:sub(#info.main_repo + 1)
  end
  return filepath
end
...
function M:get_all(dir)
  -- When querying by dir (cwd_only mode), translate the worktree dir
  ...
  for index, item in ipairs(result) do
    ...
    -- Translate canonical paths back to the current worktree prefix
    -- so files open in the right place and dedup works with ripgrep results.
    item.path = from_canonical(item.path)
...
function M:handle_open(original_filepath, batch_mode, now)
  ...
  -- Canonicalize: store under the main repo prefix so all worktrees
  -- share the same frecency entries.
  filepath = to_canonical(filepath)
