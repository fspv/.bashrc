...
local function safe_close(handle)
  if not vim.uv.is_closing(handle) then
    vim.uv.close(handle)
  end
end
...
local function spawn(cmd, opts, input, onexit)
  ...
  handle = vim.uv.spawn(cmd, args, function(code, signal)
  ...
  vim.uv.read_start(stdout, input.stdout)
  -- read child process output to stderr
  vim.uv.read_start(stderr, input.stderr)
