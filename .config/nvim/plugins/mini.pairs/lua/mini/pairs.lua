...
---
--- - Usage should be through making appropriate mappings using
---   |MiniPairs.map()| or in |MiniPairs.setup()| (for global mapping),
---   |MiniPairs.map_buf()| (for buffer mapping).
---
...
---   Note: these mappings are autocreated if they do not override existing
---   ones.
---
--- What it doesn't do:
--- - Provide smart behavior, like based on bracket balance. The default
---   behavior is to almost always perform an action (insert pair, jump over).
---   Manually
...
---     - `:lua MiniPairs.unmap_buf(0, 'i', <*>, <pair>)` - unmap key `<*>`
---       while unregistering `<pair>` pair in current buffer. Note: this
---       reverts mapping done by |MiniPairs.map_buf()|. If mapping was done
---       with |MiniPairs.map()|, unmap for buffer in usual Neovim manner:
...
--- # Disabling ~
---
--- To disable, set `vim.g.minipairs_disable` (globally) or
--- `vim.b.minipairs_disable` (for a buffer) to `true`. Considering high number
--- of different scenarios and customization intentions, writing exact rules
--- for disabling module's functionality is left to user. See
--- |mini.nvim-disabling-recipes| for common recipes.
---@tag MiniPairs

---@alias __pairs_neigh_pattern string|nil Pattern for two neighborhood
---   characters.
---   Character "\r" indicates line start, "\n" - line end.
---@alias __pairs_pair string String with two characters representing pair.
---@alias __pairs_unregistered_pair string Pair which should be unregistered
---   from both <BS> and <CR>.
...
MiniPairs.setup = function(config)
  ...
  if vim.fn.has("nvim-0.10") == 0 then
    vim.notify(
      "(mini.pairs) Neovim<0.10 is soft deprecated"
...
---     action function. Will be matched against two character neighborhood
---     (might contain multibyte characters).
...
MiniPairs.map_buf = function(buffer, mode, lhs, pair_info, opts)
  ...
  vim.api.nvim_buf_set_keymap(
    ...
    mode,
    lhs,
    H.pair_info_to_map_rhs(pair_info),
    opts
  )
  H.register_pair(
    pair_info,
    mode,
    buffer == 0 and vim.api.nvim_get_current_buf() or buffer
  )
...
---
--- Note: this only reverts mapping done by |MiniPairs.map_buf()|. If mapping
--- was done with |MiniPairs.map()|, revert to default behavior for buffer: >lua
---
...
--- results into "open" symbol. Otherwise, it pastes whole pair and moves
--- inside pair
--- with |<Left>|.
---
--- Used inside |MiniPairs.map()| and |MiniPairs.map_buf()| for an actual
--- mapping.
---
...
MiniPairs.close = function(pair, neigh_pattern)
  ...
  local move_right = not H.is_disabled()
    and H.neigh_match(neigh_pattern)
    and H.get_neigh("right") == close
...
MiniPairs.closeopen = function(pair, neigh_pattern)
  local move_right = not H.is_disabled()
    and H.get_neigh("right") == H.get_close_char(pair)
  return move_right and H.get_arrow_key("right", true)
    or MiniPairs.open(pair, neigh_pattern)
end
...
--- registered as a result of calling |MiniPairs.map()| or
--- |MiniPairs.map_buf()|.
---
...
MiniPairs.bs = function(key)
  ...
  local do_extra = not H.is_disabled()
    and H.is_pair_registered(neigh, vim.fn.mode(), "bs")
...
MiniPairs.cr = function(key)
  ...
  if
    H.is_disabled() or not H.is_pair_registered(neigh, vim.fn.mode(), "cr")
  then
    return res
  end
  ...
  H.with_temp_option(
    "eventignore",
    "InsertLeave,InsertLeavePre,InsertEnter,TextChanged,ModeChanged"
  )
...
H.default_pair_info =
  { neigh_pattern = "..", register = { bs = true, cr = true } }
...
H.register_pair = function(pair_info, mode, buffer)
  -- Process new mode
  H.registered_pairs[mode] = H.registered_pairs[mode]
    or { all = { bs = {}, cr = {} } }
...
H.ensure_cr_bs = function(mode)
  ...
  if mode == "i" and has_any_cr_pair and vim.fn.maparg("<CR>", mode) == "" then
    local opts =
      { expr = true, replace_keycodes = false, desc = "MiniPairs <CR>" }
...
H.get_neigh = function(neigh_type)
  ...
  local line = is_command_mode and vim.fn.getcmdline()
    or vim.api.nvim_get_current_line()
  line = "\r" .. line .. "\n"
  -- Get start character index accounting for added '\r' at the start
  local start = is_command_mode and vim.fn.charidx(line, vim.fn.getcmdpos())
    or vim.fn.charcol(".")
  start = start - 1

  return vim.fn.strcharpart(
    line,
    start + (neigh_type == "right" and 1 or 0),
