...
---
--- - Configurable styles: "glyph" (icon glyphs) or "ascii" (non-glyph
---   fallback).
---
...
---   with plugins outside 'mini.nvim'. See
---   |MiniIcons.mock_nvim_web_devicons()|.
---
--- - Tweaking built-in maps for "LSP kind" to include icons. In particular,
---   this makes |mini.completion| use icons in LSP step. See
...
---
--- - For file icons prefer using full path instead of relative or only
---   basename. It makes a difference if path matches pattern that uses parent
---   directories.
...
--- `{}` with your `config` table). It will create global Lua table `MiniIcons`
--- which you can use for scripting or manually (with `:lua MiniIcons.*`).
---
...
---       color scheme.
---
---     - This module prefers richer set of `nf-md-*`
---       (from "Material design" set)
...
---       in |mini.files|). This is due to using |vim.filetype.match()|
---       fallback and is present only during first call, as value is cached
---       for later uses.
---
---     - This module supports different icon styles (like "ascii" for when
---       using glyphs is not possible), while 'nvim-web-devicons' does not.
---
---     - This module provides |MiniIcons.mock_nvim_web_devicons()| function
---       which when called imitates installed 'nvim-web-devicons' plugin to
---       support
...
---           common use case of always showing an icon instead or near some
---           data.
---           There is a third returned value indicating if output is a result
---           of
...
---           preferring faster file extension resolution over filetype
---           matching.
---
---         - This module caches all its return values resulting in really fast
---           next same argument calls, while 'nvim-web-devicons' doesn't do
---           that.
---
...
---           cases for "operating system", while this module has better
---           eventual
...
---     - Provides icons only for `CompletionItemKind`, while this module also
---       has icons for `SymbolKind` and other non-LSP categories.
---     - Provides dedicated formatting function for 'hrsh7th/nvim-cmp' while
---       this
...
MiniIcons.setup = function(config)
  ...
  if vim.fn.has("nvim-0.10") == 0 then
    vim.notify(
      "(mini.icons) Neovim<0.10 is soft deprecated"
        .. " (module works but is not supported)."
        .. " It will be deprecated after the next 'mini.nvim' release"
...
---     MiniIcons.get('file', 'file.lua') -- Has `'L'` as icon; it is resolved
---                                       -- to come from 'lua' 'extension'
---                                       -- category
---     MiniIcons.get('file', 'myfile')   -- Has `'F'` as icon; it is resolved
---                                       -- to
---                                       -- come from 'file' 'default' category
--- <
--- # Customization per category ~
---
--- The following entries can be used to customize icons for supported
--- categories:
--- - `config.default`
--- - `config.directory`
--- - `config.extension`
...
---   Changing interactively via `:lua MiniIcons.config.xxx = { ... }` does
---   not work
...
--- extensions will be considered as a source of icon data during "file"
--- category
--- resolution (see |MiniIcons.get()| for more details).
--- Default: function which always returns `true` (i.e. consider all
--- extensions).
---
--- Will be called once for the biggest suffix after dot found in the file name.
--- The arguments will be `ext` (found extension; lowercase) and `file` (input
--- for which icon is computed; as is). Should explicitly return `true` if
--- `ext` is to
...
--- is needed if there is a set up filetype detection for files with
--- recognizable extension and conflicting icons (which you want to use).
--- Note: if problematic
...
---   -- use "scm" extension to resolve as Scheme file. Here is a setup to
---   -- ignore
...
---   within target category. Category "default" is used as a fallback. Use
---   third
...
---
--- - Output is cached after the first call to increase performance of next
---   calls
...
---     Icon names:
---       - <Input>: any string, but only basename is used. Works with not
---         present
...
---       - <Built-in>: popular extensions without associated filetype plus a
---         set
...
---     Icon names:
---       - <Input>: any string. Works with not present paths (no check is
---         done).
---       - <Built-in>: popular file names not tied to language/software
---         (with few notable exceptions like Neovim, Git, etc.) plus a set
---         which
...
---           - Only recognizable extensions (i.e. not default fallback) are
---             used.
...
---     Icon names:
---       - <Input>: any string.
---       - <Built-in>: any filetype that is reasonably used in Neovim
---         ecosystem.
...
--- <
---@param name string Icon name within category. Use |MiniIcons.list()| to get
---  icon
...
MiniIcons.get = function(category, name)
  ...
  name = category == "file" and name
    or (category == "directory" and H.fs_basename(name) or name:lower())
...
---
---@return table Array of icon names which are explicitly supported for
---   category.
---   Note, that `'file'` and `'extension'` categories support much more icon
---   names via their fallback to using |vim.filetype.match()| with `'filetype'`
---   category.
...
---
--- Call this function to mock exported functions of
...
---   MiniIcons.mock_nvim_web_devicons()
--- <
--- Works without installed 'nvim-web-devicons' and even with it installed
--- (needs
...
MiniIcons.mock_nvim_web_devicons = function()
  ...
  M.get_icon = function(name, ext, opts)
    -- Preferring 'name' first leads to a slightly different behavior compared
    -- to
    ...
    local icon, hl, is_default =
      MiniIcons.get(category, is_file and name or ext)
  ...
  local make_icon_tbl = function(category, name, output_name)
    ...
    return {
      icon = icon,
      color = get_hex(hl),
      cterm_color = tostring(get_cterm(hl)),
      name = output_name,
    }
  end
...
---   of time on startup. Call it lazily. For example, with
---   |MiniDeps.later()|: >lua
---
---     require('mini.icons').setup()
...
H.init_cache = function(config)
  ...
  local categories =
    { "directory", "filetype", "extension", "file", "lsp", "os" }

  H.cache, H.cache_index, H.cache_index_lookup = { default = {} }, {}, {}
  for _, cat in ipairs(categories) do
    -- Set "default" category
    local icon_def, hl_def =
      H.resolve_icon_data("default", cat, config.default[cat])
  ...
  local icon_def_def, hl_def_def =
    H.resolve_icon_data("default", "default", config.default.default)
...
H.resolve_icon_data = function(category, name, icon_data)
  ...
  return H.style_icon(has_glyph and glyph or builtin_glyph, name),
