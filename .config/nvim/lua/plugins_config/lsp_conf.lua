...
local on_attach_func = function(client, bufnr)
  ...
  vim.keymap.set({
    "n",
    "v",
  }, "<leader>ca", "<cmd>Lspsaga code_action<CR>", { buffer = bufnr, noremap = true, desc = "Code Action" })
  vim.keymap.set(
    "n",
    "gp",
  ...
  end, { buffer = bufnr, noremap = true, desc = "Jump to the Next Diagnostics" })

  vim.keymap.set("n", "[d", function()
    vim.diagnostic.jump({ count = -1, float = { border = "single", source = true } })
  end, { buffer = bufnr, noremap = true, desc = "Jump to the Previous Diagnostics" })
