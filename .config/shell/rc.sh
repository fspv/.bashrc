...
path_push_left() {
    if [ -d "$1" ]; then
        ...
        local d new_path="" IFS=':'
        for d in $PATH; do
            [ "$d" = "$1" ] && continue
            new_path="${new_path:+$new_path:}$d"
        done
        export PATH="$1${new_path:+:$new_path}"
    fi
}
...
path_push_right() {
    if [ -d "$1" ]; then
        ...
        export PATH="${new_path:+$new_path:}$1"
    fi
}
...
path_push_right "${HOME}/.cargo/bin"
path_push_right "${HOME}/go/bin"
...
scratch() {
    local ext="${1:-py}"
    local f
    f=$(mktemp "${TMPDIR:-/tmp}/scratch-XXXXXX") && mv "$f" "$f.$ext" && f="$f.$ext"
    ${EDITOR:-vim} "$f"
    echo "$f"
}
...
ld_library_path_push_left() {
    if [ -d "$1" ]; then
        ...
        export LD_LIBRARY_PATH="$1${new_path:+:$new_path}"
    fi
}
...
ld_library_path_push_right() {
    if [ -d "$1" ]; then
        ...
        export LD_LIBRARY_PATH="${new_path:+$new_path:}$1"
    fi
}
...
# MacOS-specific: `/usr/libexec/path_helper` from `/etc/profile` messes stuff up there
export PATH_HELPER_ROOT="/opt/homebrew"
